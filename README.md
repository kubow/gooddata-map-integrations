...
-   An example code for GoodData Rect SDK in the [`App.jsx`](./src/App.jsx) file.
...
In [`App.jsx`](./src/App.jsx) file, we need to:
...
1. Create a new component that renders the same pie chart as a table. Create a new file `./src/MyTable.jsx`:
...
2. Import the newly created component in the [App.jsx](./src/App.jsx):
    ```diff
    -   import { InsightView } from "@gooddata/sdk-ui-ext";
    +   import { MyTable } from "./MyTable";
    ```
3. Import the newly created component in the [App.jsx](./src/App.jsx):
    ```diff
    -   <InsightView insight={Md.Insights.ProductCategoriesPieChart} />
    +   <MyTable />
    ```
...
theme to our visualization by editing [`App.jsx`](./src/App.jsx):
...
4. Update the `App.jsx`. Since we've switched to your own data, the reference to the insight in `App.jsx` is no longer valid.
   Select a new insight to render from the catalog and update `App.jsx`:
    ```diff
...
1. Edit `./src/backend.js` file to use "bear" backend instead of "tiger" one:
    ```diff
...
5. Update the `App.jsx`. Since we've switched to your own data, the reference to the insight in `App.jsx` is no longer valid.
   Select a new insight to render from the catalog and update `App.jsx`:
    ```diff
    -   <InsightView insight={Md.Insights.ProductCategoriesPieChart} showTitle />
    +   <InsightView insight={Md.Insights.<your-insight-id>} showTitle />
