...
interface LeafletDataLoaderProps {
    workspace: string;
}

/**
 * LeafletDataLoader - Demonstrates how to extract geo data from GoodData's semantic layer
 * and prepare it for display on a Leaflet map.
 *
 ...
 * 3. Pass the processed data to the LeafletMap component for visualization
 *
 * Pattern: Include lat/lng display forms in dimensions so their values
 * appear in the slice headers.
 */
const LeafletDataLoader: React.FC<LeafletDataLoaderProps> = ({ workspace }) => {
    ...
    if (error) {
        ...
        return (
            <div className="error-container">
                ...
                <p>
                    Make sure your catalog.ts is properly configured with location attributes and you have
    ...
    const mapPoints: MapDataPoint[] = slices
        .map((slice: any) => {
            ...
            const value = dataPoints[0]?.rawValue ? parseFloat(String(dataPoints[0].rawValue)) : undefined;
    ...
    console.log("Leaflet Map Points:", mapPoints);

    return <LeafletMap points={mapPoints} />;
};

export default LeafletDataLoader;
