...
interface MapLibreDataLoaderProps {
  workspace: string;
  viewMode: "markers" | "heatmap";
}

/**
 * MapLibreDataLoader - Demonstrates how to extract geo data from GoodData's semantic layer
 * and prepare it for display on a MapLibre GL map.
 *
 ...
 * 3. Pass the processed data to the MapLibreMap component for visualization
 *
 * Pattern: Include lat/lng display forms in dimensions so their values
 * appear in the slice headers.
 */
const MapLibreDataLoader: React.FC<MapLibreDataLoaderProps> = ({
  workspace,
  viewMode,
}) => {
  ...
  if (slices.length > 0) {
    ...
    console.log("📊 First slice example:", {
  ...
  console.log("🗺️  MapLibre Data Points:", mapPoints);
  console.log("🗺️  Total valid points:", mapPoints.length);
  
  if (mapPoints.length > 0) {
    console.log("🗺️  Sample point:", mapPoints[0]);
  }

  return <MapLibreMap points={mapPoints} viewMode={viewMode} />;
};

export default MapLibreDataLoader;
